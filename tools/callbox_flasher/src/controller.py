...
class ProvisionController:
    ...
    def _run_provisioning(
        ...
        try:
            ...
            if mode in (ProvisionMode.FULL, ProvisionMode.BASELINE_ONLY, ProvisionMode.CONFIG_ONLY):
                manifest.verify_assets()

            app_info = None
            if mode in (ProvisionMode.FULL, ProvisionMode.APP_ONLY):
                if application is None:
                    raise ValueError("Chưa chọn file firmware ứng dụng")
                app_info = self._validator(application)

            nvs_bin_path = None
            if mode in (ProvisionMode.FULL, ProvisionMode.CONFIG_ONLY):
            ...
            if mode == ProvisionMode.FULL:
                ...
                curr_msg = "Đang ghi firmware và cấu hình" if nvs_bin_path else "Đang ghi firmware"
                curr_prog = 35
                emit(ProvisionEvent(curr_state, curr_msg, curr_prog))
                ...
                curr_msg = "Đang xác minh dữ liệu"
                curr_prog = 85
                emit(ProvisionEvent(curr_state, curr_msg, curr_prog))
                if nvs_bin_path is not None:
            ...
            if mode == ProvisionMode.FULL:
                ...
                ver_str = app_info.version if app_info else manifest.baseline_version
            elif mode == ProvisionMode.APP_ONLY:
