...
from tools.callbox_flasher.src.release_update import apply_verified_update
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if "--self-test" in argv:
        ...
        try:
            ...
            print(
                f"Self-test OK: Setup CallBox v{__version__}, firmware v{manifest.firmware_version}, build {manifest.baseline_version}, "
                f"{len(manifest.images) + 1} images verified, stub flasher OK, NVS generator OK."
            )
            return 0
        except Exception as error:
