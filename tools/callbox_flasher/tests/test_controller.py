...
class ProvisionControllerTests(unittest.TestCase):
    ...
    def test_firmware_bundle_writes_full_package_without_erasing_nvs(self):
        controller = self.make_controller()
        thread = controller.start("COM7", None, self.events.append, mode=ProvisionMode.FIRMWARE_BUNDLE)
        thread.join(2)
        self.assertEqual(self.adapter.calls, ["probe", "write", "verify", "reset"])
        self.assertNotIn("erase", self.adapter.calls)
        self.assertNotIn("write_config", self.adapter.calls)
        self.assertEqual(self.events[-1].state, ProvisionState.SUCCEEDED)
        self.assertEqual(self.events[-1].result.mode, ProvisionMode.FIRMWARE_BUNDLE)
        self.assertIn("1-CLICK", self.events[-1].message)
        self.assertFalse(controller.busy)

    def test_firmware_bundle_writes_all_images_without_erase_or_nvs(self):
        controller = self.make_controller()
        thread = controller.start("COM7", None, self.events.append, mode=ProvisionMode.FIRMWARE_BUNDLE)
        thread.join(2)
        self.assertEqual(self.adapter.calls, ["probe", "write", "verify", "reset"])
    ...
    def test_factory_provision_writes_boot_app_nvs_and_verifies(self):
        controller = self.make_controller()
        cfg = DeviceConfig(callbox_id="099", wifi_ssid="factory", mqtt_broker="broker")
        thread = controller.start(
            "COM7",
            None,
            self.events.append,
        ...
        self.assertEqual(self.adapter.calls, ["probe", "erase", "write", "verify", "reset"])
        self.assertEqual(self.events[-1].state, ProvisionState.SUCCEEDED)
        self.assertIn("NẠP XƯỞNG THÀNH CÔNG", self.events[-1].message)
        self.assertFalse(controller.busy)

    def test_factory_provision_without_config_is_rejected_before_erase(self):
        controller = self.make_controller()
        thread = controller.start(
            "COM7",
            FIXTURE,
        ...
        thread.join(2)
        self.assertEqual(self.events[-1].state, ProvisionState.FAILED)
        self.assertIn("Nạp xưởng", self.events[-1].message)
        self.assertNotIn("erase", self.adapter.calls)
        self.assertFalse(controller.busy)
